...
BEGIN
  ...
  -- If user is a parent, automatically grant 1-Week (7 days) Free Trial
  IF v_role = 'parent' THEN
    INSERT INTO subscriptions(user_id, plan_type, status, trial_start, trial_end)
    VALUES (NEW.id, 'free_trial', 'trial', NOW(), NOW() + INTERVAL '7 days')
    ON CONFLICT DO NOTHING;

    -- Send 7-day free trial welcome notification
    INSERT INTO notifications(user_id, title, body, type, data)
    VALUES (
      NEW.id,
      '🎁 7-Day Free Trial Activated!',
      'Welcome to BusTracker! Your 1-week free trial is active. Enjoy live bus tracking!',
      'subscription',
      jsonb_build_object('action', 'trial_started', 'days', 7)
    )
    ON CONFLICT DO NOTHING;
  END IF;
