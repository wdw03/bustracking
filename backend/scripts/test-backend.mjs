...
async function runTests() {
  try {
    ...
    if (planErr) {
      console.log(`     ⚠️ planErr: ${planErr.message}`);
    }

    assert(!planErr && plans && plans.length >= 3, `Subscription Plans seeded in DB (Found ${plans?.length || 0} active plans)`);
    if (plans && plans.length > 0) {
      console.log(`     Plans: ${plans.map(p => `${p.name} (₹${p.price})`).join(", ")}`);
    }

    // Verify critical tables exist by querying schema
    const tablesToCheck = [
      ...
      "subscriptions", "push_tokens", "notifications", "bus_proximity_events", "audit_logs"
    ];
    ...
    const { data: leakAudit, error: auditErr } = await clientAnon
      .from("audit_logs")
      .select("*");
    
    assert(leakAudit === null || leakAudit.length === 0, "Anon client cannot read audit_logs (RLS Protected)");
    ...
    assert(leakContacts === null || leakContacts.length === 0, "Anon client cannot read authorized_contacts (RLS Protected)");

    // ──────────────────────────────────────────────────────────
    // TEST 3: Edge Function — register-user (Phone Authorization)
    // ──────────────────────────────────────────────────────────
    ...
    // ──────────────────────────────────────────────────────────
    // TEST 4: Edge Function — subscription-check (Cron)
    // ──────────────────────────────────────────────────────────
    ...
    if (failed === 0) {
      console.log("🎉 ALL LIVE BACKEND TESTS PASSED! PRODUCTION READY!");
    } else {
